...
def data_uri_to_cv2_img(encoded_string):
    ...
    npimg = np.frombuffer(jpg_original, dtype=np.uint8)
    img = cv2.imdecode(npimg, cv2.IMREAD_COLOR )
    return img

def readFacesFromImg(data):
    ...
    for k, d in enumerate(dets):
        desc = "Detection {}: Left: {} Top: {} Right: {} Bottom: {}".format(k, d.left(), d.top(), d.right(), d.bottom())
        ...
        result[desc] = list(face_descriptor)
    return result

# # with open("C:\\Users\\Daniel\\Pictures\\test\\34529_1521842046732_5405828_n.jpg", "rb") as image_file:
# #     encoded_string = base64.b64encode(image_file.read())
# #     st = str(encoded_string)
# print (readFacesFromImg(base64_image))